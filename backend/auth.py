...
def create_access_token(user_id: int, email: str) -> str:
    ...
    payload = {"sub": str(user_id), "email": email, "exp": expires, "type": "access"}
...
def create_refresh_token(user_id: int) -> Tuple[str, datetime]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO refresh_tokens (user_id, token, expires_at) VALUES (?, ?, ?)", (user_id, token, expires.isoformat())
    )
    conn.commit()
    conn.close()
...
def authenticate_user(email: str, password: str) -> Tuple[bool, str, Optional[Dict]]:
    ...
    conn = sqlite3.connect(str(AUTH_DB_PATH))
    cursor = conn.cursor()
    cursor.execute("SELECT id, email, password_hash, name, is_active FROM users WHERE email = ?", (email.lower().strip(),))
    result = cursor.fetchone()
    ...
    # Update last_login
    cursor.execute("UPDATE users SET last_login = ? WHERE id = ?", (datetime.now(timezone.utc).isoformat(), user_id))
    conn.commit()
    conn.close()

    logger.info(f"Login successful: {email}")
    return True, "Login successful", {"id": user_id, "email": user_email, "name": name}
...
def get_user_by_id(user_id: int) -> Optional[Dict]:
    ...
    conn = sqlite3.connect(str(AUTH_DB_PATH))
    cursor = conn.cursor()
    cursor.execute("SELECT id, email, name, created_at, last_login, is_active FROM users WHERE id = ?", (user_id,))
    result = cursor.fetchone()
    conn.close()
...
def delete_saved_query(user_id: int, query_id: int) -> bool:
    ...
    conn = sqlite3.connect(str(AUTH_DB_PATH))
    cursor = conn.cursor()
    cursor.execute("DELETE FROM saved_queries WHERE id = ? AND user_id = ?", (query_id, user_id))
...
class AuthManager:
    ...
    def register_user(self, username: str, email: str, password: str) -> Tuple[bool, Any]:
        ...
        if success and user_id:
            ...
            return True, {"user_id": user_id, "username": username, "email": email, "token": token, "message": message}
        else:
    ...
    def login_user(self, username: str, password: str) -> Tuple[bool, Any]:
        ...
        return True, {"user_id": user["id"], "username": user["name"], "email": user["email"], "token": token}
