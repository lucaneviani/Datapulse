...
def analyze_dataframe(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    suggestions.append({
        "type": "table",
        "title": "Dati Completi",
        "config": {"columns": list(df.columns)[:10]}
    })
...
DASHBOARD_TEMPLATES = {
    "sales_overview": {
        ...
        "widgets": [
            ...
            {"type": "bar_chart", "title": "Vendite per Regione", "query": "SELECT region, SUM(total) FROM customers c JOIN orders o ON c.id = o.customer_id GROUP BY region"},
            {"type": "pie_chart", "title": "Ordini per Segmento", "query": "SELECT segment, COUNT(*) FROM customers c JOIN orders o ON c.id = o.customer_id GROUP BY segment"},
            {"type": "line_chart", "title": "Trend Mensile", "query": "SELECT strftime('%Y-%m', order_date), SUM(total) FROM orders GROUP BY 1"}
        ]
    },
    "customer_analysis": {
        ...
        "widgets": [
            ...
            {"type": "bar_chart", "title": "Clienti per Regione", "query": "SELECT region, COUNT(*) FROM customers GROUP BY region"},
            {"type": "pie_chart", "title": "Segmentazione", "query": "SELECT segment, COUNT(*) FROM customers GROUP BY segment"}
        ]
    },
    "product_performance": {
        ...
        "widgets": [
            ...
            {"type": "bar_chart", "title": "Top 10 Prodotti", "query": "SELECT p.name, SUM(oi.sales) FROM products p JOIN order_items oi ON p.id = oi.product_id GROUP BY p.id ORDER BY 2 DESC LIMIT 10"},
            {"type": "pie_chart", "title": "Vendite per Categoria", "query": "SELECT category, SUM(sales) FROM products p JOIN order_items oi ON p.id = oi.product_id GROUP BY category"}
        ]
    }
}
